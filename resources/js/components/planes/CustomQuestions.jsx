            ...
            <div className="pt-2">
                <Shine enableOnHover color="white" opacity={0.3} duration={600} asChild>
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all bg-gray-200 text-gray-900 border border-gray-300 hover:bg-gray-200 hover:text-gray-900 hover:border-gray-300"
                    >
            ...
            <AnimatePresence>
                {questions.map((q) => (
                    <motion.div
                        ...
                        className="rounded-xl bg-white border border-gray-300 p-3 space-y-2"
                    >
                        <div className="flex items-center gap-2">
                            <span className="flex-1 text-xs font-medium text-gray-900">{q.question_text}</span>
                            <button
                        ...
                        <input
                            ...
                            className="w-full bg-gray-200 border border-gray-300 rounded-lg px-3 py-2 text-xs text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-[#208DCA]/40 focus:ring-1 focus:ring-[#208DCA]/30 transition-colors"
                        />
                    </motion.div>
                ))}
            </AnimatePresence>
                ...
                <AnimatePresence>
                    {showPicker && (
                        <motion.div
                            ...
                            className="absolute bottom-full left-0 mb-2 w-80 bg-white border border-gray-300 rounded-xl shadow-2xl overflow-hidden z-50"
                        >
                            {/* Search */}
                            ...
                            <div className="max-h-48 overflow-y-auto">
                                {availableQuestions.map((q) => (
                                    <button
                                        ...
                                        className="w-full flex items-center gap-2 px-3 py-2 text-xs text-gray-900 hover:bg-gray-200 hover:text-gray-900 transition-colors text-left"
                                    >
                                        {q.is_template
                                            ? <Star size={11} className="text-amber-400 flex-shrink-0" fill="currentColor" />
                                            : <MessageSquarePlus size={11} className="text-gray-900 flex-shrink-0" />
                                        }
                                ...
                                {availableQuestions.length === 0 && !canCreate && (
                                    <p className="px-3 py-3 text-[11px] text-gray-900 text-center">
                            ...
                            {canCreate && (
                                <button
                                    ...
                                    disabled={loading}
                                    className="w-full flex items-center gap-2 px-3 py-2.5 text-xs text-[#208DCA] hover:bg-[#208DCA]/8 transition-colors border-t border-gray-300"
                                >
                                    <Plus size={12} />
