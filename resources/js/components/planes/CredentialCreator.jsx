    ...
    return (
        <div className="rounded-2xl border border-slate-200 bg-slate-200 overflow-hidden">
            <div className="flex gap-0">
                ...
                <div className="flex-shrink-0 p-5 flex flex-col items-center gap-3 border-r border-slate-200 bg-black/20">
                    <p className="text-[10px] text-slate-900 uppercase tracking-wider">Vista previa</p>
                    ...
                    <div className="rounded-xl overflow-hidden shadow-2xl shadow-black/60 relative">
                        ...
                        {!photoImgRef.current && (
                            <div className="absolute pointer-events-none"
                                ...
                                <span className="text-[8px] text-slate-900">↑ foto</span>
                            </div>
                        )}
                    </div>
                    ...
                    {activeTarget && (activeTx !== null) && (photoImgRef.current || logoImgRef.current) && (
                        <div className="w-full rounded-xl bg-slate-200 border border-slate-200 p-3 space-y-2">
                            <div className="flex items-center justify-between">
                                <span className="text-[10px] text-slate-900 uppercase tracking-wider flex items-center gap-1">
                                    <Move size={9} />
                                    {activeTarget === "photo" ? "Foto" : "Logo"} — arrastrar para mover
                                </span>
                                <button onClick={resetActiveTx}
                                    className="text-[10px] text-slate-900 hover:text-slate-900 flex items-center gap-1 transition-colors"
                            ...
                            <div className="flex items-center gap-2">
                                <ZoomOut size={11} className="text-slate-900 flex-shrink-0" />
                                ...
                                />
                                <ZoomIn size={11} className="text-slate-900 flex-shrink-0" />
                                <span className="text-[10px] text-slate-900 w-8 text-right">
                            ...
                            <div className="flex gap-1">
                                {photoImgRef.current && (
                                    <button onClick={() => setActiveTarget("photo")}
                                        className={`flex-1 text-[10px] py-1 rounded-lg transition-colors ${activeTarget === "photo" ? "bg-[#208DCA]/20 text-[#208DCA]" : "text-slate-900 hover:text-slate-900 hover:bg-slate-200"}`}>
                                        Ajustar foto
                                    </button>
                                )}
                                {logoImgRef.current && (
                                    <button onClick={() => setActiveTarget("logo")}
                                        className={`flex-1 text-[10px] py-1 rounded-lg transition-colors ${activeTarget === "logo" ? "bg-[#208DCA]/20 text-[#208DCA]" : "text-slate-900 hover:text-slate-900 hover:bg-slate-200"}`}>
                                        Ajustar logo
                                    </button>
                                )}
                            </div>
                    ...
                    {(photoImgRef.current || logoImgRef.current) && (
                        <p className="text-[9px] text-slate-900 text-center leading-relaxed">
                    ...
                    <div className="flex gap-2 w-full">
                        <button onClick={() => photoInputRef.current?.click()}
                            className="flex-1 flex flex-col items-center gap-1 py-2 rounded-xl border border-dashed border-slate-200 text-slate-900 hover:text-slate-900 hover:border-[#208DCA]/30 transition-all group relative overflow-hidden min-h-[60px]">
                            {photoPreview
                    ...
                    <div className="flex gap-2 w-full">
                        <button onClick={download}
                            className="flex-1 flex items-center justify-center gap-1.5 text-xs py-2 rounded-xl bg-slate-200 border border-slate-200 text-slate-900 hover:text-slate-900 hover:bg-white/12 transition-all">
                            <Download size={12} />
                            PNG
                        </button>
                        {uuid && (
                ...
                <div className="flex-1 flex flex-col min-w-0">
                        ...
                        {[
                            ...
                            <button key={id} onClick={() => setActiveTab(id)}
                                className={`flex items-center gap-1.5 px-4 py-3 text-xs font-medium border-b-2 transition-all ${
                                    activeTab === id ? "border-[#208DCA] text-[#208DCA]" : "border-transparent text-slate-900 hover:text-slate-900"}`}>
                                <Icon size={12} />
                                {label}
                            </button>
                        ))}
                    ...
                    <div className="flex-1 p-4 space-y-4 overflow-y-auto max-h-80">
                        ...
                        {activeTab === "datos" && (<>
                            <div>
                                <label className="text-[10px] font-semibold text-slate-900 mb-1 block uppercase tracking-wide">Nombre *</label>
                                <input value={config.name}
                                    onChange={e => setConfig(c => ({ ...c, name: e.target.value }))}
                                    placeholder="Nombre completo del portador"
                                    className="w-full bg-slate-200 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-[#208DCA]/50 focus:bg-slate-200 transition-all"
                                />
                            </div>
                            <div>
                                <label className="text-[10px] font-semibold text-slate-900 mb-1 block uppercase tracking-wide">Cargo / Tipo de acreditación</label>
                            ...
                            <label className="flex items-center gap-2 cursor-pointer">
                                ...
                                <span className="text-xs text-slate-900">Mostrar logo</span>
                            </label>
                        </>)}
                        ...
                        {activeTab === "diseño" && (<>
                            <div>
                                <label className="text-[10px] font-semibold text-slate-900 mb-2 block uppercase tracking-wide">Paletas</label>
                                <div className="grid grid-cols-4 gap-1.5">
                                    {COLOR_PRESETS.map(p => (
                                        <button key={p.label}
                                            onClick={() => setConfig(c => ({ ...c, bg: p.bg, accent: p.accent, textColor: p.text }))}
                                            className="flex flex-col items-center gap-1 p-1.5 rounded-lg hover:bg-slate-200 transition-colors group">
                                            <div className="w-10 h-6 rounded-md overflow-hidden flex border border-slate-200">
                                                <div className="flex-1" style={{ backgroundColor: p.bg }} />
                                                <div className="w-[10px]" style={{ backgroundColor: p.accent }} />
                                            </div>
                                            <span className="text-[9px] text-slate-900 group-hover:text-slate-900">{p.label}</span>
                                        </button>
                                    ))}
                                </div>
                            </div>
                            <div className="grid grid-cols-3 gap-3">
                                {[
                                    ...
                                    <div key={key}>
                                        <label className="text-[10px] text-slate-900 mb-1 block">{label}</label>
                                        <div className="flex items-center gap-2">
                                            <input type="color" value={config[key]}
                                                onChange={e => setConfig(c => ({ ...c, [key]: e.target.value }))}
                                                className="w-8 h-8 rounded-lg cursor-pointer border-0 bg-transparent" />
                                            <span className="text-[10px] text-slate-900 font-mono">{config[key]}</span>
                                        </div>
                                    </div>
                                ))}
                                <div>
                                    <label className="text-[10px] text-slate-900 mb-1 block">Texto</label>
                                    <div className="flex gap-1">
                                        {[{ v: "#ffffff", l: "Blanco" }, { v: "#1a1a1a", l: "Negro" }].map(({ v, l }) => (
                                            <button key={v} onClick={() => setConfig(c => ({ ...c, textColor: v }))}
                                                className={`flex-1 py-1.5 rounded-lg text-[10px] border transition-all ${config.textColor === v ? "bg-slate-200 border-white/30 text-white" : "border-slate-200 text-slate-900 hover:text-slate-900"}`}>
                                                {l}
                                            </button>
                                        ))}
                                    </div>
                        ...
                        {/* ── Campos ── */}
                        {activeTab === "campos" && (<>
                            <p className="text-xs text-slate-900">Campos adicionales (máx. 4)</p>
                                ...
                                {config.extraFields.map((field, idx) => (
                                    <div key={idx} className="flex items-center gap-2">
                                        <input value={field.label}
                                            ...
                                            className="w-28 bg-slate-200 border border-slate-200 rounded-lg px-2 py-1.5 text-xs text-slate-900 placeholder-white/20 focus:outline-none focus:border-[#208DCA]/50 transition-all"
                                        />
                                        <input value={field.value}
                                            onChange={e => updateField(idx, "value", e.target.value)}
                                            placeholder="Valor"
                                            className="flex-1 bg-slate-200 border border-slate-200 rounded-lg px-2 py-1.5 text-xs text-slate-900 placeholder-white/20 focus:outline-none focus:border-[#208DCA]/50 transition-all"
                                        />
                                        <button onClick={() => setConfig(c => ({ ...c, extraFields: c.extraFields.filter((_, i) => i !== idx) }))}
                                            className="p-1 text-slate-900 hover:text-red-400 transition-colors flex-shrink-0">
                                            <X size={13} />
                                        </button>
                                    </div>
                                ))}
