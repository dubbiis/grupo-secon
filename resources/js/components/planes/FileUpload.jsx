...
export default function FileUpload({
    ...
    const getIcon = (mime) => {
        ...
        return <File size={13} className="text-slate-900" />;
    };
            ...
            <AnimatePresence>
                {visible.map((f) => (
                    <motion.div
                        ...
                        className={`flex items-center gap-2.5 px-3 py-2 bg-slate-200 border border-slate-200 rounded-xl text-xs ${f._optimistic ? "opacity-60" : ""}`}
                    >
                        {getIcon(f.mime_type)}
                        <span className="flex-1 truncate text-slate-900">{f.original_name}</span>
                        {f._optimistic
                        ...
                        {!f._optimistic && (
                            <button
                                ...
                                className="text-slate-900 hover:text-red-400 transition-colors ml-1 flex-shrink-0"
                            >
                                <X size={12} />
                            </button>
                        )}
            ...
            <div
                ...
                className={`flex flex-col items-center justify-center gap-2.5 border-2 border-dashed rounded-2xl px-6 py-8 cursor-pointer transition-all group ${
                    dragOver
                        ...
                        : "border-slate-200 hover:border-[#208DCA]/40 hover:bg-[#208DCA]/4"
                }`}
            >
                <div className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-all ${
                    dragOver
                        ...
                        : "bg-slate-200 border-slate-200 group-hover:bg-[#208DCA]/10 group-hover:border-[#208DCA]/20"
                }`}>
                    {uploading
                        ? <CloudUpload size={16} className="text-[#208DCA] animate-bounce" />
                        : <CloudUpload size={16} className="text-slate-900 group-hover:text-[#208DCA] transition-colors" />
                    }
                </div>
