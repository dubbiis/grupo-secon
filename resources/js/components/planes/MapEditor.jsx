...
const ICON_CATEGORIES = {
    ...
    ubicaciones: {
        label: "Ubicaciones", color: "#253C87",
        icons: [
    ...
    return (
        <div className="flex flex-col gap-3 w-full" onClick={() => setContextMenu(null)}>
            ...
            <div className="flex flex-wrap items-center gap-2 p-3 rounded-2xl bg-white/3 border border-white/8">
                ...
                {/* Colors */}
                <div className="flex items-center gap-1 pr-3 border-r border-white/10 flex-wrap">
                    {QUICK_COLORS.map((c) => (
                        <button
                            key={c}
                            onClick={() => setColor(c)}
                ...
                <div className="flex items-center gap-1 pr-3 border-r border-white/10">
                    {STROKE_WIDTHS.map((w) => (
                        <button key={w} onClick={() => setStrokeWidth(w)} title={`${w}px`}
                            className={`flex items-center justify-center w-7 h-7 rounded-lg transition-all ${strokeWidth === w ? "bg-white/15" : "hover:bg-white/8"}`}
                        >
                ...
                {["rect", "circle"].includes(tool) && (
                    <button
                        ...
                        className={`flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg border transition-all ${
                            useFill ? "bg-[#208DCA]/20 border-[#208DCA]/40 text-[#208DCA]" : "bg-white/5 border-white/10 text-white/40 hover:text-white"
                        }`}
                    >
                    ...
                    <div className="flex items-center gap-1 pr-3 border-r border-white/10">
                        {TEXT_SIZES.map((s) => (
                            <button key={s} onClick={() => setTextSize(s)} title={`${s}px`}
                                className={`text-xs px-1.5 py-0.5 rounded transition-all ${textSize === s ? "bg-white/15 text-white" : "text-white/35 hover:text-white hover:bg-white/8"}`}
                ...
                <div className="flex items-center gap-2 pr-3 border-r border-white/10">
                    ...
                    />
                    <span className="text-[10px] text-white/40 w-6">{Math.round(opacity * 100)}%</span>
                </div>
                ...
                <div className="relative pr-3 border-r border-white/10">
                    <button
                        ...
                        className="flex items-center gap-1 text-xs text-white/50 hover:text-white px-2 py-1.5 rounded-lg hover:bg-white/8 transition-colors"
                    >
                        <Layers size={13} />
                        Iconos
                ...
                <div className="flex items-center gap-0.5 pr-3 border-r border-white/10">
                    <button onClick={undo} disabled={!canUndo} title="Deshacer (Ctrl+Z)"
                        className="w-7 h-7 rounded-lg flex items-center justify-center text-white/40 hover:text-white hover:bg-white/8 disabled:opacity-20 transition-all">
                        <Undo2 size={13} />
                    </button>
                ...
                <div className="flex items-center gap-1 pr-3 border-r border-white/10">
                    <button onClick={() => setShowGrid((v) => !v)} title="Cuadrícula"
                        className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all ${showGrid ? "text-[#208DCA] bg-[#208DCA]/15" : "text-white/30 hover:text-white hover:bg-white/8"}`}>
                        <Grid size={13} />
                    </button>
                    ...
                    <button onClick={() => setZoom(1)} title="Zoom 100%"
                        className="text-[10px] text-white/30 hover:text-white px-1 transition-colors">
                        {Math.round(zoom * 100)}%
                    </button>
                ...
                <button onClick={() => setShowMap((v) => !v)}
                    className={`flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg border transition-all ${
                        showMap ? "bg-[#208DCA]/15 border-[#208DCA]/30 text-[#208DCA]" : "bg-white/5 border-white/10 text-white/40 hover:text-white"
                    }`}
                >
                    <Map size={12} />
                    Mapa
                            ...
                            {mode === "standalone" && (
                                <div className="flex items-center gap-1 text-xs">
                                    <button onClick={() => setExportFormat("png")}
                                        className={`px-2 py-1 rounded-l-lg border border-white/10 transition-colors ${exportFormat === "png" ? "bg-white/15 text-white" : "text-white/30 hover:text-white"}`}>
                                        PNG
                                    </button>
                                    <button onClick={() => setExportFormat("jpeg")}
                                        className={`px-2 py-1 rounded-r-lg border border-white/10 transition-colors ${exportFormat === "jpeg" ? "bg-white/15 text-white" : "text-white/30 hover:text-white"}`}>
                                        JPG
                                    </button>
                                </div>
                            )}
                            <button onClick={copyToClipboard} title="Copiar al portapapeles"
                                className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-all ${copiedMsg ? "bg-green-600/20 border-green-500/30 text-green-400" : "bg-white/5 border-white/10 text-white/40 hover:text-white hover:bg-white/10"}`}>
                                {copiedMsg ? <Check size={13} /> : <Copy size={13} />}
                            </button>
                            <RippleButton size="sm" onClick={handleSave} disabled={saving}
                                className={`gap-1.5 text-xs border-0 ${saved ? "bg-green-600 hover:bg-green-600" : "bg-gradient-to-r from-[#253C87] to-[#208DCA]"} text-white`}>
                    ...
                    {!hasBg && (
                        ...
                        >
                            <div className="text-center space-y-2">
                                <FileImage size={40} className="text-white/20 mx-auto" />
                                <p className="text-sm font-medium text-white/50">Sube una imagen o captura de pantalla</p>
                                <p className="text-xs text-white/25">Arrastra aquí · Ctrl+V para pegar · clic para seleccionar</p>
                            </div>

                            <div className="flex items-center gap-2 flex-wrap justify-center px-8">
                                <span className="text-xs text-white/20">— o empezar con lienzo en blanco —</span>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2 justify-center px-8">
                                {CANVAS_PRESETS.map((p) => (
                                    <button
                                        ...
                                        className="text-xs px-3 py-1.5 rounded-xl bg-white/6 border border-white/10 text-white/50 hover:text-white hover:bg-white/10 transition-colors"
                                    >
                                        {p.label}
                                        <span className="text-white/25 ml-1.5 text-[10px]">{p.w}×{p.h}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
            ...
            <AnimatePresence>
                {contextMenu && (
                    <motion.div
                        ...
                        className="fixed z-50 bg-[#0f1219] border border-white/12 rounded-xl shadow-2xl py-1 min-w-[160px]"
                        style={{ left: contextMenu.x, top: contextMenu.y }}
                        onClick={(e) => e.stopPropagation()}
                    >
                        {[
                            { label: "Eliminar",        action: () => { deleteSelected(); setContextMenu(null); }, cls: "text-red-400 hover:bg-red-500/10" },
                            { label: "Duplicar",         action: () => { duplicateElement(contextMenu.elIdx); setContextMenu(null); }, cls: "text-white/70 hover:bg-white/8" },
                            { label: "Traer al frente",  action: () => { bringToFront(contextMenu.elIdx); setContextMenu(null); }, cls: "text-white/70 hover:bg-white/8" },
                            { label: "Enviar al fondo",  action: () => { sendToBack(contextMenu.elIdx); setContextMenu(null); }, cls: "text-white/70 hover:bg-white/8" },
            ...
            {/* ── Tips ── */}
            <div className="flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-white/20 px-1">
                <span>Teclas: <kbd className="bg-white/8 px-1 rounded">P</kbd>lápiz <kbd className="bg-white/8 px-1 rounded">L</kbd>ínea <kbd className="bg-white/8 px-1 rounded">A</kbd>flecha <kbd className="bg-white/8 px-1 rounded">R</kbd>ect <kbd className="bg-white/8 px-1 rounded">C</kbd>írculo <kbd className="bg-white/8 px-1 rounded">T</kbd>exto <kbd className="bg-white/8 px-1 rounded">S</kbd>eleccionar</span>
                <span>·</span>
