    ...
    return (
        ...
        >
            <div className="px-8 py-6 max-w-4xl mx-auto">
                <div className="mb-2">
                    <Link href="/admin/prompts" className="text-xs text-gray-900 hover:text-gray-900 transition-colors">
                        ← Volver a Prompts
                    </Link>
                </div>
                ...
                <form onSubmit={submit} className="space-y-6">
                    {/* Model + tokens */}
                    <div className="p-5 rounded-2xl bg-white border border-gray-300 space-y-5">
                        <div className="flex items-center gap-2 pb-4 border-b border-gray-300">
                            ...
                            <p className="text-xs font-semibold text-gray-900 uppercase tracking-wide">Configuración del modelo IA</p>
                        </div>

                        <div className="grid grid-cols-2 gap-5">
                            {/* Modelo */}
                            <div className="space-y-2">
                                <label className="text-xs font-semibold text-gray-900 block uppercase tracking-wide">Modelo GPT</label>
                                <select
                                    value={data.model}
                                    onChange={(e) => setData("model", e.target.value)}
                                    className="flex h-9 w-full rounded-lg border border-gray-300 bg-gray-200 px-3 py-1 text-sm text-gray-900 shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#208DCA]/50"
                                >
                                ...
                                <div className="space-y-1.5 pt-1">
                                    {[
                                        ...
                                        <div key={name} className={`flex gap-2 p-2 rounded-lg transition-colors ${data.model === name ? "bg-[#208DCA]/8 border border-[#208DCA]/15" : "opacity-40"}`}>
                                            ...
                                            <span className="text-[10px] text-gray-900 leading-snug">{desc}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            {/* Max tokens */}
                            <div className="space-y-2">
                                <label className="text-xs font-semibold text-gray-900 block uppercase tracking-wide">Máximo de tokens</label>
                                <Input
                                    type="number"
                                ...
                                <div className="space-y-2 pt-1">
                                    <p className="text-[10px] text-gray-900 leading-relaxed">
                                        Un <span className="text-gray-900 font-medium">token</span> equivale aproximadamente a ¾ de una palabra en español. El modelo detiene la generación al alcanzar este límite.
                                    </p>
                                    <div className="space-y-1">
                                        {[
                                            ...
                                            <div key={range} className="flex items-start gap-2">
                                                <span className="font-mono text-[10px] text-gray-900 w-24 flex-shrink-0 pt-px">{range}</span>
                                                <div>
                                                    <span className="text-[10px] text-white/55 font-medium">{label}</span>
                                                    <span className="text-[10px] text-gray-900"> — {hint}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                    ...
                    {/* Variables */}
                    <div className="p-5 rounded-2xl bg-white border border-gray-300">
                        <div className="flex items-center gap-2 mb-3">
                            <Code2 size={13} className="text-[#208DCA]" />
                            <p className="text-xs font-semibold text-gray-900 uppercase tracking-wide">Variables disponibles — clic para insertar</p>
                        </div>
                    ...
                    {/* System prompt */}
                    <div className="p-5 rounded-2xl bg-white border border-gray-300 space-y-3">
                        <div className="space-y-2">
                            <label className="text-xs font-semibold text-gray-900 block uppercase tracking-wide">System Prompt</label>
                            <p className="text-xs text-gray-900 leading-relaxed">
                                Es la <span className="text-gray-900 font-medium">instrucción invisible</span> que recibe el modelo antes de leer el mensaje del usuario. Define su rol, tono y restricciones. El usuario final nunca lo ve.
                            </p>
                            <div className="flex flex-wrap gap-2 text-[10px]">
                                <span className="px-2 py-1 rounded-md bg-gray-200 border border-gray-300 text-gray-900">✓ Define el rol ("Eres un experto en...")</span>
                                <span className="px-2 py-1 rounded-md bg-gray-200 border border-gray-300 text-gray-900">✓ Idioma y formato de salida</span>
                                <span className="px-2 py-1 rounded-md bg-gray-200 border border-gray-300 text-gray-900">✓ Tono (formal, técnico, conciso)</span>
                                <span className="px-2 py-1 rounded-md bg-gray-200 border border-gray-300 text-gray-900">✗ No pongas datos del evento aquí</span>
                            </div>
                        </div>
                    ...
                    <div className="rounded-2xl bg-white border border-gray-300 overflow-hidden">
                        ...
                        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-300">
                            <div className="space-y-2">
                                <label className="text-xs font-semibold text-gray-900 uppercase tracking-wide">User Prompt Template</label>
                                <p className="text-xs text-gray-900 leading-relaxed max-w-lg">
                                    El mensaje que recibe el modelo <span className="text-gray-900 font-medium">con los datos reales del evento</span> ya insertados. Escribe el texto del prompt y usa <code className="bg-gray-200 px-1.5 py-0.5 rounded text-[#208DCA] font-mono">{"{{variable}}"}</code> donde quieras que aparezca un dato del formulario.
                                </p>
                            </div>
                            <button
                                type="button"
                                ...
                                className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-all ${
                                    showPreview
                                        ...
                                        : "bg-white border-gray-300 text-gray-900 hover:text-gray-900 hover:bg-gray-200"
                                }`}
                            >
                                    ...
                                    >
                                        <div className="rounded-xl border border-[#208DCA]/20 bg-[#208DCA]/4 overflow-hidden">
                                            <div className="flex items-center gap-2 px-4 py-2.5 border-b border-[#208DCA]/15 bg-[#208DCA]/6">
                                                ...
                                                <span className="ml-auto text-xs text-gray-900 font-mono">Concierto Aitana · Madrid Arena 2025</span>
                                            </div>
                                            <pre className="px-5 py-4 text-xs text-gray-900 leading-relaxed whitespace-pre-wrap font-sans overflow-y-auto max-h-96">
                                                {preview}
                                            </pre>
                                        </div>
                                    </motion.div>
                    ...
                    <div className="rounded-2xl bg-white border border-gray-300 overflow-hidden">
                        {/* Header */}
                        <div className="px-5 pt-5 pb-4 border-b border-gray-300 space-y-2">
                            <div className="flex items-center gap-2">
                                <BookOpen size={13} className="text-amber-400" />
                                <label className="text-xs font-semibold text-gray-900 uppercase tracking-wide">Ejemplo de texto de salida</label>
                                <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/10 border border-amber-500/20 text-amber-400">Opcional</span>
                            </div>
                            <p className="text-xs text-gray-900 leading-relaxed max-w-2xl">
                            ...
                            <div className={`rounded-xl border p-4 transition-colors ${
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <div className="space-y-1.5 flex-1">
                                        <div className="flex items-center gap-2">
                                            <Zap size={12} className={data.use_example_output ? "text-amber-400" : "text-gray-900"} />
                                            <span className={`text-xs font-semibold ${data.use_example_output ? "text-amber-300" : "text-gray-900"}`}>
